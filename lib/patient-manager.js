...
export default class PatientManager {
  ...
  createPatient() {
    let patient_view = new CreatePatientView({'new': true});
    patient_view.open();
  }
  ...
  newSession() {
    ...
    if (this.current_session_date == null) {
      let date = new Date();
      let datestring = (date.getDate().toString().length == 1 ? "0" : "") + date.getDate() + "." + (date.getMonth().toString().length == 1 ? "0" : "") + date.getMonth() + "." + date.getFullYear();
      let timestring = (date.getHours().toSring().length == 1 ? "0" : "") + date.getHours() + ":" + (date.getMonth().toString().length == 1 ? "0" : "") + date.getMinutes();
