...
export default class CreatePatientView {
  ...
  save() {
    ...
    let patient_dir = path.join(atom.config.get('homeo-atom.patientRoot'), this.firstname + "_" + this.lastname);

    if (!fs.existsSync(patient_dir)) {
      fs.mkdirSync(patient_dir);
    } else {
      atom.notifications.addWarning("PatientIn unter diesem Namen existiert bereits! <br>Erstellung fehlgeschlagen.", {});
      return;
    }

    fs.writeFileSync(path.join(patient_dir, "data.json"), JSON.stringify(json_patient, null, 4));

    atom.notifications.addSuccess("PatientIn wurde gespeichert!", {});
  }
