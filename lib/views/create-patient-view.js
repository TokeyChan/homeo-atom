...
export default class CreatePatientView {
  ...
  constructor(options) {
    this.firstname = options['new'] ? "" : options['firstname'];
    this.lastname = options['new'] ? "" : options['lastname'];
    this.gender = options['new'] ? "" : options['gender'];
    this.birthday = options['new'] ? "" : options['birthday'];
    this.street = options['new'] ? "" : options['street'];
    this.city = options['new'] ? "" : options['city'];
    this.phone = options['new'] ? "" : options['phone'];
    this.email = options['new'] ? "" : options['email'];

    this.is_new = options['new'];
    if (!this.is_new)
      this.id = options['id'];
  ...
  save() {
    ...
    this.email = this.emailEditor.getText();
    ...
    if (this.is_new) {
      ...
      var data = patients_data['patients'].filter(pat => pat['id'] == this.id)[0];
      let patient_dir = path.join(atom.config.get('homeo-atom.patientRoot'), this.id + "-" + this.firstname + "_" + this.lastname);
      let json_patient = {
        'firstname': this.firstname,
        'lastname': this.lastname,
  ...
  create_html() {
    ...
    this.nameEditor.setText(this.firstname);
    this.nameEditor.readOnly = !this.is_new;

    let lastname_tag = document.createElement('div');
    lastname_tag.textContent = 'Nachname:';

    this.lastnameEditor = new TextEditor({ mini: true});
    this.lastnameEditor.setText(this.lastname);
    this.lastnameEditor.readOonlnly = !this.is_new;
    ...
    this.birthdayEditor.setText(this.birthday);
    ...
    let email_tag = document.createElement('div');
    email_tag.textContent = "Email:";

    this.emailEditor = new TextEditor({mini: true});
    this.emailEditor.setText(this.email);
    ...
    this.element.appendChild(email_tag);
    ...
    let editors = [this.nameEditor.element, this.lastnameEditor.element, this.birthdayEditor.element, this.streetEditor.element, this.cityEditor.element, this.phoneEditor.element, this.emailEditor.element];
    ...
    this.choose_gender(this.gender);
  }
}
